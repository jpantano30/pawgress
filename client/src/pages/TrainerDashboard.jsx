...
import { SkeletonStats, SkeletonList } from '../components/shared/SkeletonCard';
...
export default function TrainerDashboard() {
  ...
  async function loadData() {
    ...
    try {
      ...
      if (!localStorage.getItem('pawgress_welcomed_trainer')) setShowWelcome(true);
    } catch (err) {
  ...
  return (
    <div style={{ maxWidth:900, margin:'0 auto' }}>
      ...
      <div style={{ marginBottom:28 }}>
        ...
        {inviteCode && (
          <div style={{ display:'inline-flex', alignItems:'center', gap:10, marginTop:10, background:'var(--white)', border:'1px solid var(--gray-border)', borderRadius:'var(--radius-sm)', padding:'8px 16px', boxShadow:'var(--card-shadow)', flexWrap:'wrap' }}>
            ...
            <button onClick={copyCode} style={{ background: copied ? 'var(--teal)' : 'var(--coral-light)', color: copied ? 'white' : 'var(--coral)', border:'none', borderRadius:'var(--radius-sm)', padding:'4px 12px', fontSize:12, cursor:'pointer', fontFamily:'var(--font-sans)', fontWeight:500 }}>
              {copied ? '✓ Copied!' : 'Copy'}
            </button>
          </div>
        )}
      ...
      {loading ? (
        ...
        <>
          ...
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:14 }}>
            ...
            <button onClick={() => setShowAddDog(true)} style={{ background:'var(--coral)', color:'white', border:'none', borderRadius:'var(--radius-sm)', padding:'8px 18px', fontSize:13, fontWeight:500, cursor:'pointer', fontFamily:'var(--font-sans)' }}>+ Add Dog</button>
          </div>
          ...
          {dogs.length === 0 ? (
            <div style={{ border:'2px dashed var(--gray-border)', borderRadius:'var(--radius-md)', padding:48, textAlign:'center', background:'var(--white)' }}>
              ...
              <button onClick={() => setShowAddDog(true)} style={{ background:'var(--coral)', color:'white', border:'none', borderRadius:'var(--radius-sm)', padding:'10px 22px', fontSize:14, fontWeight:500, cursor:'pointer', fontFamily:'var(--font-sans)' }}>+ Add Your First Dog</button>
            </div>
          ) : (
            <div className="dog-grid" style={{ display:'grid', gridTemplateColumns:'repeat(2,1fr)', gap:14 }}>
              {dogs.map((dog, i) => (
                <div key={dog.id} onClick={() => navigate(`/dogs/${dog.id}`)}
                  ...
                  <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom: dog.notes ? 10 : 0 }}>
                    ...
                    <div style={{ flex:1, minWidth:0 }}>
                      <div style={{ fontFamily:'var(--font-serif)', fontSize:16, color:'var(--brown)', fontWeight:500, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{dog.name}</div>
